...
class HeatTransportData(PROCESSModelData):
    ...
    pacpmw: Parameter[float] = 0.0
    """total pulsed power system load (MW)"""

    peakmva: Parameter[float] = 0.0
    """peak MVA requirement"""

    p_fw_div_heat_deposited_mw: Parameter[float] = 0.0
    """heat removal from first wall/divertor (MW)"""

    p_plant_electric_gross_mw: Parameter[float] = 0.0
    """gross electric power (MW)"""

    p_hcd_electric_loss_mw: Parameter[float] = 0.0
    """power dissipated in heating and current drive system (MW)"""

    p_hcd_electric_total_mw: Parameter[float] = 0.0
    """injector wall plug power (MW)"""

    p_hcd_secondary_electric_mw: Parameter[float] = 0.0
    """Secondary HCD system injector wall plug power (MW)"""

    p_hcd_primary_electric_mw: Parameter[float] = None
    """Primary HCD system injector wall plug power (MW)"""

    p_plant_electric_net_mw: Parameter[float] = 0.0
    """net electric power (MW)"""

    p_plant_electric_recirc_mw: Parameter[float] = 0.0
    """recirculating electric power (MW)"""

    priheat: Parameter[float] = 0.0
    """total thermal power removed from fusion core (MW)"""

    p_div_secondary_heat_mw: Parameter[float] = 0.0
    """Low-grade heat lost in divertor (MW)"""

    p_hcd_secondary_heat_mw: Parameter[float] = 0.0
    """Low-grade heat lost into HCD apparatus (MW)"""

    p_plant_secondary_heat_mw: Parameter[float] = 0.0
    """Low-grade heat (MW)"""

    p_shld_secondary_heat_mw: Parameter[float] = 0.0
    """Low-grade heat deposited in shield (MW)"""

    p_plant_primary_heat_mw: Parameter[float] = 0.0
    """High-grade heat useful for electric production (MW)"""

    pflux_plant_floor_electric: Parameter[float] = 150.0
    """base AC power requirement per unit floor area (W/m2)"""

    p_tf_electric_supplies_mw: Parameter[float] = 0.0
    """total steady state TF coil AC power demand (MW)"""

    tlvpmw: Parameter[float] = 0.0
    """estimate of total low voltage power (MW)"""

    p_tritium_plant_electric_mw: Parameter[float] = 15.0
    """power required for tritium processing (MW)"""

    temp_turbine_coolant_in: Parameter[float] = 0.0
    """coolant temperature at turbine inlet (K) (`i_thermal_electric_conversion = 3,4`)"""

    vachtmw: Parameter[float] = 0.5
    """vacuum pump power (MW)"""

    f_p_plant_electric_recirc: Parameter[float] = 0.0
