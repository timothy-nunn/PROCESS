...
class TFData(PROCESSModelData):
    ...
    dx_tf_turn_conduit_full_average: Parameter[float] = 0.0
    """Average full width of the conduit surrounding the TF turn cable space [m]"""

    dx_tf_turn_general: Parameter[float] = 0.0
    ...
    t_turn_tf_max: Parameter[float] = 0.05
    ...
    dx_tf_turn_cable_space_general: Parameter[float] = 0.0
    ...
    j_tf_bus: Parameter[float] = 1.25e6
    """bussing current density (A/m2)"""

    j_crit_str_tf: Parameter[float] = 0.0
    ...
    j_crit_str_0: Parameter[list[float]] = field(
        default_factory=lambda: np.array([
            596905475.80390120,
            1925501534.8512938,
            724544682.96063495,
    ...
    eyoung_steel: Parameter[float] = 2.05e11
    """Steel case Young's modulus (Pa) (default value from DDD11-2 v2 2 (2009))"""

    eyoung_cond_axial: Parameter[float] = 6.6e8
    ...
    str_wp_max: Parameter[float] = 0.7e-2
    ...
    rho_tf_joints: Parameter[float] = 2.5e-10
    ...
    th_joint_contact: Parameter[float] = 0.03
    """TF sliding joints contact pad width [m]"""

    p_tf_joints_resistive: Parameter[float] = 0.0
    """Calculated TF joints resistive power losses [W]"""

    len_tf_coil: Parameter[float] = 0.0
    """TF coil circumference (m)"""

    eff_tf_cryo: Parameter[float] = -1.0
    ...
    dthet: Parameter[list[float]] = field(default_factory=lambda: np.zeros(4))
    """angle of arc i (rad)"""

    radctf: Parameter[list[float]] = field(default_factory=lambda: np.zeros(4))
    """radius of arc i (m)"""

    r_tf_arc: Parameter[list[float]] = field(default_factory=lambda: np.zeros(5))
    """x location of arc point i on surface (m)"""

    xctfc: Parameter[list[float]] = field(default_factory=lambda: np.zeros(4))
    """x location of arc centre i (m)"""

    z_tf_arc: Parameter[list[float]] = field(default_factory=lambda: np.zeros(5))
    """y location of arc point i on surface (m)"""

    yctfc: Parameter[list[float]] = field(default_factory=lambda: np.zeros(4))
    ...
    tfa: Parameter[list[float]] = field(default_factory=lambda: np.zeros(4))
    """Horizontal radius of inside edge of TF coil (m)"""

    tfb: Parameter[list[float]] = field(default_factory=lambda: np.zeros(4))
    ...
    tcpav2: Parameter[float] = 0.0
    """Computed centrepost average temperature (K) (for consistency)"""

    temp_tf_legs_outboard: Parameter[float] = -1.0
