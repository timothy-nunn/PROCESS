...
class PFCoilData(PROCESSModelData):
    ...
    ricpf: Parameter[float] = 0.0

    ssq0: Parameter[float] = 0.0

    stress_z_cs_self_peak_midplane: Parameter[float] = 0.0
    """Peak axial stress (z) in central solenoid at midplane due to its own field (when at peak current) (Pa)"""

    stress_z_cs_self_midplane_profile: Parameter[list[float]] = field(
    ...
    stress_z_cs_self_profile: Parameter[list[float]] = field(
    ...
    z_pf_coil_middle_group_array: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros((N_PF_GROUPS_MAX, N_PF_COILS_IN_GROUP_MAX))
    )
    """2D array of PF coil middle heights, indexed by group and coil in group"""

    ccls: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros(N_PF_GROUPS_MAX)
    )

    ccl0: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros(N_PF_GROUPS_MAX)
    )

    bpf2: Parameter[list[float]] = field(default_factory=lambda: np.zeros(NGC2))

    vsdum: Parameter[list[float]] = field(default_factory=lambda: np.zeros((NGC2, 3)))
    ...
    ccls_ma: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros(N_PF_GROUPS_MAX)
    )
    ...
    c_pf_cs_coil_flat_top_ma: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros(NGC2)
    )
    ...
    c_pf_cs_coil_pulse_end_ma: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros(NGC2)
    )
    ...
    j_crit_str_cs: Parameter[float] = 0.0
    ...
    j_crit_str_pf: Parameter[float] = 0.0
    """superconductor strand critical current density under operating
    ...
    f_z_cs_tf_internal: Parameter[float] = 0.71
    """Central solenoid height / TF coil internal height"""

    f_a_cs_turn_steel: Parameter[float] = 0.5
    """Fraction of CS turn poloidal area that is steel (`iteration variable 122`)"""

    pf_current_safety_factor: Parameter[float] = 1.0
    ...
    pfcaseth: Parameter[list[float]] = field(default_factory=lambda: np.zeros(NGC2))
    """steel case thickness for PF coil i (m)"""

    rho_pf_coil: Parameter[float] = 2.5e-8
    """PF coil resistivity (if i_pf_conductor=1/RESISTIVE) (Ohm-m)"""

    rhopfbus: Parameter[float] = 3.93e-8
    ...
    m_pf_coil_max: Parameter[float] = 0.0
    """mass of heaviest PF coil (tonnes)"""

    r_pf_coil_outer_max: Parameter[float] = 0.0
    """radius of largest PF coil (m)"""

    p_pf_electric_supplies_mw: Parameter[float] = 0.0
    """Total mean wall plug power dissipated in PFC and CS power supplies (MW) (issue #713)"""

    p_cs_resistive_flat_top: Parameter[float] = 0.0
    """central solenoid resistive power during flattop (W)"""

    p_pf_coil_resistive_total_flat_top: Parameter[float] = 0.0
    """total PF coil resistive losses during flattop (W)"""

    r_pf_coil_inner: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros(NGC2)
    )
    """inner radius of coil i (m)"""

    r_pf_coil_outer: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros(NGC2)
    )
    """outer radius of coil i (m)"""

    c_pf_cs_coils_peak_ma: Parameter[list[float]] = field(
        default_factory=lambda: np.zeros(NGC2)
    )
    """peak current in coil i (MA-turns)"""

    j_pf_coil_wp_peak: Parameter[list[float]] = field(
        default_factory=lambda: np.full(NGC2, 3.0e7)
    )
    ...
    b_cs_limit_max: Parameter[float] = 13.0
    """Central solenoid max field limit [T]"""

    f_dr_dz_cs_turn: Parameter[float] = 70.0 / 22.0
    """Ratio of CS coil turn conduit length to depth"""

    dr_cs_turn: Parameter[float] = 0.0
    """Length of CS coil turn conduit"""

    dr_cs_full: Parameter[float] = 0.0
    """Full radial thickness of the central solenoid (m)"""

    dz_cs_turn: Parameter[float] = 0.0
    """Depth/width of CS coil turn conduit"""

    radius_cs_turn_corners: Parameter[float] = 3.0e-3
    """Radius of curvature of CS coil turn corners (m)"""

    radius_cs_turn_cable_space: Parameter[float] = 0.0
    """Length of CS coil turn conduit length"""

    r_pf_outside_tf_midplane: Parameter[float] = 0.0
