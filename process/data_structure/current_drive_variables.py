...
class CurrentDriveData(PROCESSModelData):
    """Dataclass holding current drive variables"""

    dx_beam_duct: Parameter[float] = 0.58
    ...
    big_q_plasma: Parameter[float] = 0.0
    """Fusion gain; P_fusion / (P_injection + P_ohmic)"""

    f_c_plasma_bootstrap: Parameter[float] = 0.0
    """bootstrap current fraction (enforced; see i_bootstrap_current)"""

    f_c_plasma_bootstrap_max: Parameter[float] = 0.9
    """Maximum fraction of plasma current from bootstrap"""

    f_c_plasma_bootstrap_iter89: Parameter[float] = 0.0
    """bootstrap current fraction, ITER 1989 model"""

    f_c_plasma_bootstrap_nevins: Parameter[float] = 0.0
    """bootstrap current fraction, Nevins et al model"""

    f_c_plasma_bootstrap_sauter: Parameter[float] = 0.0
    """bootstrap current fraction, Sauter et al model"""

    f_c_plasma_bootstrap_wilson: Parameter[float] = 0.0
    """bootstrap current fraction, Wilson et al model"""

    f_c_plasma_bootstrap_sakai: Parameter[float] = 0.0
    """Bootstrap current fraction, Sakai et al model"""

    f_c_plasma_bootstrap_aries: Parameter[float] = 0.0
    """Bootstrap current fraction, ARIES model"""

    f_c_plasma_bootstrap_andrade: Parameter[float] = 0.0
    """Bootstrap current fraction, Andrade et al model"""

    f_c_plasma_bootstrap_hoang: Parameter[float] = 0.0
    """Bootstrap current fraction, Hoang et al model"""

    f_c_plasma_bootstrap_wong: Parameter[float] = 0.0
    """Bootstrap current fraction, Wong et al model"""

    bscf_gi_i: Parameter[float] = 0.0
    """Bootstrap current fraction, first Gi et al model"""

    bscf_gi_ii: Parameter[float] = 0.0
    """Bootstrap current fraction, second Gi et al model"""

    f_c_plasma_bootstrap_sugiyama_l: Parameter[float] = 0.0
    """Bootstrap current fraction, L-mode Sugiyama et al model"""

    f_c_plasma_bootstrap_sugiyama_h: Parameter[float] = 0.0
    """Bootstrap current fraction, H-mode Sugiyama et al model"""

    cboot: Parameter[float] = 1.0
    """bootstrap current fraction multiplier"""

    c_beam_total: Parameter[float] = 0.0
    """neutral beam current (A)"""

    f_c_plasma_diamagnetic_hender: Parameter[float] = 0.0
    """diamagnetic current fraction, Hender fit"""

    f_c_plasma_diamagnetic_scene: Parameter[float] = 0.0
    """diamagnetic current fraction, SCENE fit"""

    f_c_plasma_diamagnetic: Parameter[float] = 0.0
    """diamagnetic current fraction"""

    p_hcd_ecrh_injected_total_mw: Parameter[float] = 0.0
    """ECH power (MW)"""

    p_ebw_injected_mw: Parameter[float] = 0.0
    """Electron bernstein power (MW)"""

    p_hcd_ecrh_electric_mw: Parameter[float] = 0.0
    """ECH wall plug power (MW)"""

    p_hcd_ebw_electric_mw: Parameter[float] = 0.0
    """Electron bernstein wall plug power (MW)"""

    p_hcd_icrh_electric_mw: Parameter[float] = 0.0
    """Ion cyclotron wall plug power (MW)"""

    eta_cd_hcd_primary: Parameter[float] = 0.0
    """Current drive efficiency of primary HCD system (A/W)"""

    eta_cd_hcd_secondary: Parameter[float] = 0.0
    """Current drive efficiency of secondary HCD system (A/W)"""

    c_hcd_primary_driven: Parameter[float] = 0.0
    """Current in plasma driven by primary HCD system (A)"""

    c_hcd_secondary_driven: Parameter[float] = 0.0
    """Current in plasma driven by secondary HCD system (A)"""

    f_c_plasma_hcd_primary: Parameter[float] = 0.0
    """Fraction of plasma current driven by primary HCD system"""

    f_c_plasma_hcd_secondary: Parameter[float] = 0.0
    """Fraction of plasma current driven by secondary HCD system"""

    n_ecrh_harmonic: Parameter[float] = 2.0
    ...
    f_p_beam_shine_through: Parameter[float] = 0.0
    """neutral beam shine-through fraction"""

    dx_beam_shield: Parameter[float] = 0.5
    """neutral beam duct shielding thickness (m)"""

    p_hcd_primary_extra_heat_mw: Parameter[float] = 0.0
    """heating power not used for current drive (MW) (`iteration variable 11`)"""

    p_hcd_secondary_extra_heat_mw: Parameter[float] = 0.0
    """secondary fixed heating power not used for current drive (MW)"""

    p_hcd_injected_max: Parameter[float] = 150.0
    """maximum allowable value for injected power (MW) (`constraint equation 30`)"""

    p_hcd_injected_electrons_mw: Parameter[float] = 0.0
    """auxiliary injected power to electrons (MW)"""

    p_hcd_injected_ions_mw: Parameter[float] = 0.0
    """auxiliary injected power to ions (MW)"""

    p_hcd_injected_total_mw: Parameter[float] = 0.0
    """total auxiliary injected power (MW)"""

    p_hcd_injected_current_total_mw: Parameter[float] = 0.0
    """total auxiliary injected power (MW)"""

    p_hcd_secondary_injected_mw: Parameter[float] = 0.0
    """secondary total fixed auxiliary injected power (MW)"""

    p_hcd_primary_injected_mw: Parameter[float] = 0.0
    """primary auxiliary injected power (MW)"""

    f_c_plasma_internal: Parameter[float] = 0.0
    """plasma current fraction driven internally (Bootstrap + Diamagnetic + PS)"""

    p_hcd_lowhyb_injected_total_mw: Parameter[float] = 0.0
    """Total lower hybrid injection power (MW)"""

    p_hcd_icrh_injected_total_mw: Parameter[float] = 0.0
    """Total ion cyclotron injection power (MW)"""

    p_hcd_ebw_injected_total_mw: Parameter[float] = 0.0
    """Total electron bernstein wave injection power (MW)"""

    p_beam_plasma_coupled_mw: Parameter[float] = None
    """Total neutral beam power that is coupled to plasma after losses (MW)"""

    p_hcd_beam_injected_total_mw: Parameter[float] = 0.0
    """neutral beam injection power (MW)"""

    p_beam_orbit_loss_mw: Parameter[float] = 0.0
    """neutral beam power lost after ionisation but before thermalisation (orbit loss power) (MW)"""

    f_c_plasma_pfirsch_schluter: Parameter[float] = 0.0
    """Pfirsch-Schlüter current fraction"""

    p_hcd_lowhyb_electric_mw: Parameter[float] = 0.0
    """lower hybrid wall plug power (MW)"""

    pwpnb: Parameter[float] = 0.0
    """neutral beam wall plug power (MW)"""

    radius_beam_tangency: Parameter[float] = 0.0
    """neutral beam centreline tangency radius (m)"""

    radius_beam_tangency_max: Parameter[float] = 0.0
    """maximum tangency radius for centreline of beam (m)"""

    n_beam_decay_lengths_core: Parameter[float] = 0.0
    """neutral beam e-decay lengths to plasma centre"""

    n_beam_decay_lengths_core_required: Parameter[float] = 3.0
